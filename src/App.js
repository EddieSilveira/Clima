  ...
  const listaErrosGeo = [
    {
      codigo: 1,
      texto:
        'Não foi dada a permissão para o sistema encontrar a sua localização',
    },
    ...
    async function obtemCidade(latitude, longitude) {
      ...
      await fetch(url)
        ...
        .catch(function (error) {
          console.error(
            `Não foi possível buscar a cidade a partir da lat/long. Erro: ${error.message}`,
          );
        });
    }
  }, []);
    ...
    <>
      ...
      {erroGeo && (
        <Alert variant="danger" onClose={() => setErroGeo(null)} dismissible>
          <Alert.Heading>
            Ops! Ocorreu um erro ao obter a sua localização.
          </Alert.Heading>
